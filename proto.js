...
chart.init = function( config ) {
	...
	this.data = processWorkingData( commonWords, exceptions, inputData, range );
...
chart.render = function ( config ) {
	...
	var index = 0
		...
		,	barColor = 0
		,	chartType = '\u2592'
		,	rainbow = [ 'red', 'green', 'white', 'yellow', 'cyan' ]
		,	barColors = (config['p'] === 'rainbow') ? rainbow : config['p'].split(',')
